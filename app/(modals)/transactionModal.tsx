...
const API_URL = "http://192.168.1.104:5000/predict";
...
const TransactionModal = () => {
  ...
  const {
    data: budget,
  } = useFetchData<WalletType>("budgets", [
    where("uid", "==", user?.uid),
  ...
  // Reset manual category override when description becomes empty
  useEffect(() => {
  ...
  const predictExpenseCategory = async (text: string) => {
    try {
      ...
      if (!response.ok) {
        console.log("Prediction API failed:", response.status);
        return null;
      }
  ...
  // Auto-predict expense category from description
  useEffect(() => {
    if (transaction.type !== "expense") return;
    if (oldTransaction?.id) return; // do not auto-change existing transaction while editing
    if (categoryManuallyChanged) return; // respect manual selection
    ...
    predictionTimer.current = setTimeout(async () => {
      ...
      const result = await predictExpenseCategory(text);
    ...
    <ModalWrapper>
      <View style={styles.container}>
        ...
        <ScrollView contentContainerStyle={styles.form} showsVerticalScrollIndicator={false}>
          ...
          {/* budget */}
          <View style={styles.inputContainer}>
            <Typo color={colors.neutral200} size={16}>Budget</Typo>
            <Dropdown
              style={styles.dropdownContainer}
              activeColor={colors.neutral700}
              ...
              data={budget.map((budget) => ({
                label: `${budget?.name} ($${budget.amount})`,
                value: budget?.id,
              }))}
              maxHeight={300}
              labelField="label"
